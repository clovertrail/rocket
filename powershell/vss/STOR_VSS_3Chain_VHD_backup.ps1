...
#######################################################################
function MyCheckRecoveringJ([string] $ipv4)
{
    $retValue = $False
	$cmdExpr = "echo y | $gPscp -i $gPPK root@${ipv4}:/var/log/messages ./boot.msg"
    gLogMsg "$cmdExpr"
...
$ipv4 = GetIPv4ViaHyperV $vmName $hvServer
# Check to see Linux VM is running VSS backup daemon 
$sts = CheckVSSDaemon $ipv4
if (-not $sts[-1])
{
...
Write-Output "new switch is $Switch"
# Now create New VM out of this VHD.
# New VM is static hardcoded since we do not need it to be dynamic
$GChildVHD = $CreateVHD[-1]

$newVm = New-VM -Name $vmNewTmp -VHDPath $GChildVHD -MemoryStartupBytes 1024MB -SwitchName $Switch
if (-not $?)
    {
...
echo "New 3 Chain VHD VM $vmNewTmp Created: Success" >> $summaryLog
Write-Output "INFO: New 3 Chain VHD VM $vmNewTmp Created"

$timeout = 500
$sts = Start-VM -Name $vmNewTmp -ComputerName $hvServer 
if (-not (WaitForVMToStartKVP $vmNewTmp $hvServer $timeout ))
{
    Write-Output "Error: ${vmName1} failed to start"
     return $False
}

Write-Output "INFO: New VM $vmNewTmp started"
...
$VMtoBackup = Get-WBVirtualMachine | where vmname -like $vmNewTmp
...
$vm = Get-VM -Name $vmNewTmp -ComputerName $hvServer
    if (-not $vm)
    {
...
# Get new IPV4
$ipv4 =  GetIPv4ViaHyperV $vmNewTmp $hvServer
if (-not $?)
    {
...
$recovery=MyCheckRecoveringJ $ipv4
if ($recovery[-1])
{
...
$sts = Stop-VM –Name $vmNewTmp -TurnOff
if (-not $?)
    {
       Write-Output "Error: Unable to Shut Down VM $vmNewTmp" 
...
$sts = Remove-VM -Name $vmNewTmp -Confirm:$false -Force
if (-not $?)
    {
      Write-Output "Error: Deleting New VM $vmNewTmp"  
    } 

Write-Output "INFO: Deleted VM $vmNewTmp"
