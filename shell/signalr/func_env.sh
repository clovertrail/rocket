...
_EOF
	nohup sh $remote_run > ${result_dir}/${script_name}.log 2>&1 &
}
...
function gen_single_websocket_script() {
	...
	if [ "$bench_type" == "service" ]
	then
		server_endpoint=${bench_service_server}:${bench_service_port}
	else
		server_endpoint=${bench_app_server}:${bench_app_port}
	fi
...
./websocket-bench -m master -a "\${agents}" -s "${server_endpoint}" -t \$${result_name} -c ${cmd_file_prefix}_${bench_codec}_${bench_name}_${bench_type} -o ${result_name}
EOF
}
